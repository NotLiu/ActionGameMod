...
with(objCloud){
	...
	if(y-player_high > 525 ||  y-player_high < -525 ){
			...
			var cloud_y = irandom_range(camera_get_view_y(view_camera[0])-50,camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) - 150);
	...
	if(inst_361EDD44.out == 1 && inst_AC7B74C.out == 1 && global.max_height > 1000){
		instance_destroy(id);
		scr_remove_cloud(id);
		objGameManager.cloud_count -= 1;
	}
